...
int32_t test_sqrt_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	s_out = BuildSqrtCircuit(s_A, s_B, (BooleanCircuit*) circ);
	...
	*/
	output = s_out->get_clear_value<double>();
...
share* BuildSqrtCircuit(share *s_A, share *s_B, BooleanCircuit *bc) {

	share* s_mult = bc->PutFPGate(s_A, SQR, no_status);

	// share* s_out = bc->PutFPGate(s_mult, s_B, ADD);
